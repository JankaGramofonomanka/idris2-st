...
data STrans
  ...
  RunAs
     : Applicative m
    => STrans m t     in_res (const out_res)
  ...
  Combine
    ...
    -> (vs : List Var)
    -> (prf : VarsIn (comp :: vs) res)
    -> STrans m () res (const (combineVarsIn res prf))
  ...
  Call
     : STrans m t sub new_f
    -> (res_prf : SubRes sub old)
...
dropEnv (z :: zs) (InRes idx rest) = let
  [e] = envElem idx (z :: zs)
  in e :: dropEnv (dropDups (z :: zs) idx) rest
...
runST env (Split lbl prf)  k
  = let
    val = lookupEnv prf env
    env' = updateEnv prf env (Value ())
  in k (mkVars val) (addToEnv val env')
...
runST env (Call prog res_prf)  k
  = let
    env' = dropEnv env res_prf
    in runST env' prog $ \prog', envk
      => k prog' (rebuildEnv envk env res_prf)
...
runWithLoop
   : Env res
  -> Fuel
  -> STransLoop IO a res resf
  -> IO (Maybe (result ** Env (resf result)))
runWithLoop env fuel prog
  = runSTLoop fuel env prog (\res, env' => pure (Just (res ** env'))) (pure Nothing)
...
st_precondition (CantUnify _ tm1 tm2 _ _ _)
  ...
  where
    ...
    renderPre got req
      = [ SubReport
          [ TextPart "Operation has preconditions: "
          , TermPart req
          ]
        , SubReport
          [ TextPart "States here are: "
          , TermPart got
          ]
        ]
    ...
    renderPost got req
      = [ SubReport
          [ TextPart "Operation has postconditions: "
          , TermPart req
          ]
        , SubReport
          [ TextPart "Required result states here are: "
          , TermPart got
          ]
        ]
